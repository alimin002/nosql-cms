...
class M_dashboard extends CI_Model {
	...
	public function getPemesananMasuk(){
		$sql="select a.id_seq from t_trx_order a";
		$result=$this->db->query($sql)->result();
		return count($result);
	}

	public function getPemesananMasukUnverified(){
		$sql="select a.id_seq from t_trx_order a where a.status_bayar=0";
		$result=$this->db->query($sql)->result();
		return count($result);
	}

	public function getPemesananMasukVerified(){
		$sql="select a.id_seq from t_trx_order a where a.status_bayar=1";
		$result=$this->db->query($sql)->result();
		return count($result);
	}
