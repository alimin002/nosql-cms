(function($){
    ...
    function toHtml(target, rows, title, detail){
        rows = rows || getRows(target);
        var dg = $(target);
        var data =['<h3>'+title+'</h3>'];
        for (var i = 0; i < detail.length; i++) {
            detail[i]
            for (var index in detail[i]) {
                data.push('' + index + ' : ' + detail[i][index] + '<br>');
            }
        }
        data.push('<br>');
        ...
        $.map(rows, function(row){
            ...
            for(var i=0; i<fields.length; i++){
                var field = fields[i];
                data.push(
                    '<td style="'+'mso-number-format:0;'+tdStyle0+'">'+row[field]+'</td>'
                );
            }
            data.push('</tr>');
        });
    ...
    function toExcel(target, param){
        ...
        var table = toHtml(target, rows, title, detail);
